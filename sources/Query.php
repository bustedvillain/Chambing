...
class Query {
    ...
    function select($tipo = "obj") {
        if (!empty($this->sql)) {
            ...
            $this->idQuery = mysql_query($this->sql, $this->idConexion)
                    or die(Error::error_mysql(mysql_errno() . ": " . mysql_error(), __FILE__, __LINE__, __CLASS__, __FUNCTION__, __METHOD__, $_SERVER['PHP_SELF'], $this->sql));
            ...
            if ($this->numRegistros() > 0) {
                if (strcmp($tipo, "obj") == 0) {
                    ...
                    while ($row = mysql_fetch_array($this->idQuery, NULL, PGSQL_BOTH)) {
                        $this->arregloArr[] = $row;
                    }
...
//                        imprimeConsola($this->sql);
    ...
    /**
     * Ejecuta un insert de varios elementos
     * @param type $tabla la tabla en la que se quiere insertar
     * @param type $campos los campos separados por comas
     * @param type $values los valores a insertar separados por comas y paréntesis
     * '(renglón1_valor1,renglón1_valor2,...),(renglón2_valor1,renglón2_valor2,...),...'
     */
    function insertMultiple($tabla = NULL, $campos = NULL, $values = NULL) {
        if (!empty($tabla) || !empty($campos) || !empty($values)) {
            unset($this->sql);
            $this->sql = "INSERT INTO $tabla ($campos) VALUES $values ";
//                        imprimeConsola($this->sql);
    ...
    function ultimoID($tabla) {
        ...
        $this->select("arr");
        $campoId = mysql_field_name($this->idQuery, 0) or die("Error");
    ...
    function numCampos() {
        return ($this->idQuery) ? mysql_num_fields($this->idQuery) : 0;
    }

    #Retorna el numero de tuplas modificadas en INSERT, UPDATE, and DELETE queries. 

    function camposAfectados() {
        return ($this->idQuery) ? mysql_affected_rows($this->idQuery) : 0;
    }
    ...
    function camposSelect() {
        if ($this->idQuery) {
            ...
            $i = mysql_num_fields($this->idQuery);
            for ($j = 0; $j < $i; $j++) {
                $fieldname = mysql_field_name($this->idQuery, $j);
